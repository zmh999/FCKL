    ...
    def test(self,
        ...
        with torch.no_grad():
            for i in range(10000):
                ...
                # print(relation['word'].shape)
                logits, pred , _ = model(batch, relation, N, K, 1 + Q * na_rate, sne=sne)
                # print(logits.shape)
                print(pred)
                newpred = pred.cpu()
                ...
                for i in list:
                    lists.append(i)
                print(len(lists))
                with open("pred-10-5-F.json", "w") as f: ####################################################
                    f.write(str(lists))
        return lists
...
class FewRelDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print('ssssss',support_set['word'])
        # print('qqqqqq',query_set['word'])
...
def main():
    ...
    parser.add_argument('--test', default='test_pubmed_input-10-5', #############################
                        help='train file')
    ...
    parser.add_argument('--max_length', default=128, type=int,
                        help='max length')
    parser.add_argument('--load_ckpt', default='checkpoint/proto-bert-train_wiki-val_pubmed-5-5-adv_pubmed_unsupervised-catentity-55F.pth.tar',
                        help='load ckpt')
